...
import { Label } from "@/components/ui/label"; // Correct label import
...
export default function Register() {
  ...
  return (
    <div className="min-h-screen flex justify-center items-center bg-gradient-to-br from-black via-zinc-900 to-black px-4">
      ...
      {/* Error Alert */}
      {error && (
        <Alert className="fixed top-6 left-1/2 -translate-x-1/2 z-50 w-full max-w-sm rounded-xl border border-red-500 bg-red-900 text-red-200 shadow-lg transition-all duration-300">
          {error}
        </Alert>
      )}
      ...
      <Card className="w-full max-w-[350px] md:max-w-md rounded-2xl bg-zinc-950 border border-zinc-800 shadow-lg">
        ...
        <CardContent className="p-6">
          <form onSubmit={handleSubmit} className="space-y-5">
            ...
            <Button
              type="submit"
              className="w-full bg-cyan-600 hover:bg-cyan-700 transition text-white"
            >
              Register
            </Button>
          </form>
